...
/* 5.1. Checkbox options: { optionId, label, isTicked } */
export const checkboxOptions = pgTable("checkbox_options", {
  optionId: text().primaryKey(),
  shapeId: text()
    .notNull()
    .references(() => checkboxShapes.shapeId, { onDelete: "cascade" }),
  label: varchar().notNull(),
  isTicked: boolean().notNull(),
...
/* 13. Divider shape: { thickness } */
export const dividerShapes = pgTable("divider", {
  shapeId: text()
    .primaryKey()
    .references(() => shapes.id, { onDelete: "cascade" }),
  thickness: integer().default(2), // Default thickness value
});
