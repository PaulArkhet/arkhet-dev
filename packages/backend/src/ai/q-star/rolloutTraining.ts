...
const candidateGenerator: CandidateGenerator<Task, IntermediateStep> = async (
  ...
  while (attempt <= maxRetries) {
    try {
      ...
      if (!response.tool_calls) throw new Error('No tool calls in response');
      ...
      if ((error as any).toString().includes('Too many tokens')) {
        if (attempt < maxRetries) {
          ...
          console.error(
            `Candidate Generation: Exceeded maximum retries for action due to "Too many tokens" error.`
          );
          throw error;
        }
      } else {
