...
export default function RadiobuttonComponent({
  ...
  return (
    <div
      className="flex hover:text-[#42A5F5] hover:bg-[#202020] rounded pt-4 transition-all ease-in-out duration-200"
      onClick={() => {
        handleAddShape({
          type: "radio",
          canvasRef,
    ...
    >
      <HoverCard openDelay={400} closeDelay={0}>
        <HoverCardTrigger>
          <button>
            ...
            <p className="text-xs pt-2 pb-2">
              <span className="font-extrabold">R</span>adio Button
            </p>
          </button>
        </HoverCardTrigger>
        <HoverCardContent
        ...
        >
          <p className="text-xs">
            Press <span className="text-sm font-extrabold">r</span> to add a
            radio button
          </p>
        </HoverCardContent>
      </HoverCard>
